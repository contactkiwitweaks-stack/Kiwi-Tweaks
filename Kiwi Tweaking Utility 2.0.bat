...
:: Instant Silent Elevation Check
net session >nul 2>&1
if %errorlevel% neq 0 (
    powershell -NoProfile -Command "(New-Object -ComObject 'Shell.Application').ShellExecute('cmd.exe', '/k `\"%~f0`\"', '', 'runas', 1)"
    exit /b
)

:: Environment Setup
color 0D
title KXWIES Free Utility 2.0
...
:: System Restore frequency configuration
...
echo %w%                          WELCOME TO KXWIES FREE UTILITY
echo %b%__________________________________________________________________________________
echo.
echo %p%Would you like to create a system restore point before proceeding? (Y/N)
echo %r%It's recommended to create a restore point so you can revert changes if needed%reset%
echo.
echo    %g%[Y]%w% Create System Restore Point       %g%[N]%w% Skip and Proceed to Utility
echo.
choice /c yn /n >nul
if errorlevel 2 goto warning
if errorlevel 1 goto rON
goto warning

:rON
chcp 437 >nul 2>&1
powershell -ExecutionPolicy Bypass -Command "Checkpoint-Computer -Description 'Kiwis Restore Point' -RestorePointType 'MODIFY_SETTINGS'"
chcp 65001 >nul 2>&1
echo.
echo.
echo.
...
echo.
echo.
echo  %w%I am not responsible for any problems/damages with your pc this may cause.
echo  %r%MAKE A RESTORE POINT!%w%
echo.
echo  %w%Any distribution, reselling etc is NOT tolerated.
echo.
echo  %w%Type numbers/letters to select your options
echo  %w%Read all warnings, popups etc... don't just blindly press buttons without reading
echo.
echo %b%__________________________________________________________________________________
echo.
echo.
...
cls
echo %b%__________________________________________________________________________________
echo %w%                             KXWIES Free Utility 2.0
echo %b%__________________________________________________________________________________
echo.
echo    %p%[1]%w%  Windows Tweaks                %p%[2]%w%  Network Tweaks
echo    %p%[3]%w%  Power Tweaks                  %p%[4]%w%  Priority Tweaks
echo    %p%[5]%w%  General Tweaks                %p%[6]%w%  BCDEdit Tweaks
echo    %p%[7]%w%  Visual Tweaks                 %p%[8]%w%  Storage Tweaks
echo    %p%[9]%w%  USB Tweaks                    %p%[10]%w% Fix Corrupted Files
echo    %p%[11]%w% RAM Tweaks                    %p%[12]%w% Fullscreen Optimization
echo    %p%[13]%w% GPU Tweaks                    %p%[14]%w% BIOS Tweaks
...
echo %b%__________________________________________________________________________________
echo.
echo %y%Please select an option from the menu above.
...
:site
start https://kiwitweaks.vercel.app/
echo.
echo.
echo.
echo.                                              %b%+=======================================================+
...
echo.
echo %b%__________________________________________________________________________________
echo %w%                Fixes and System Reverts
echo %b%__________________________________________________________________________________
echo.
echo %p%[1]%w% Reinstall All Windows Apps    %p%[2]%w% Revert Alt Tab Settings
echo.
echo %p%[3]%w% Backup and Restore System       %p%[4]%w% Revert All Tweaks to Defaults
echo.
echo %w%Type "X" to go back to menu
echo.
echo %b%__________________________________________________________________________________
set /p input=:
if /i "%input%" == "1" goto Reinstall
if /i "%input%" == "2" goto altrev
if /i "%input%" == "3" goto backup_menu
if /i "%input%" == "4" goto revert_all
if /i "%input%" == "X" cls & goto menu

echo Invalid Input & goto Misspell_Fixes
...
echo Invalid Input & goto Misspell_real
...
echo %w%                Ping
echo %b%__________________________________________________________________________________
echo %w% How to find out how much actual ping you have!
echo.
...
echo Invalid Input & goto Misspell_realfn
...
echo.
echo %b%__________________________________________________________________________________
echo %w%                Backup and Restore System (Kiwi 2.0 Engine)
echo %b%__________________________________________________________________________________
echo %w% Snapshot your registry and system settings, or restore stock Windows defaults.
echo.
echo.
echo %p%[1]%w% Create System Settings Snapshot Backup
echo.
echo %p%[2]%w% Restore from Previous Backup
echo.
echo %p%[3]%w% Revert All Tweaks to Windows Stock Defaults
echo.
echo.
echo %w% Press "X" to go back to Main Menu
echo.
echo %b%__________________________________________________________________________________
echo.
set "input="
set /p input=:
if /i "%input%" == "1" goto create_backup
if /i "%input%" == "2" goto restore_backup
if /i "%input%" == "3" goto revert_all
if /i "%input%" == "X" cls & goto menu
echo Invalid Input & goto Misspell_BackupMenu
...
cls
echo.
echo Creating Kiwi 2.0 Settings Snapshot...
if not exist "%USERPROFILE%\KiwiTweaks_Backups" mkdir "%USERPROFILE%\KiwiTweaks_Backups"
for /f "tokens=2 delims==" %%I in ('wmic os get localdatetime /value 2^>nul') do set "dt=%%I"
if "%dt%"=="" set "dt=%date:~-4%%date:~4,2%%date:~7,2%_%time:~0,2%%time:~3,2%%time:~6,2%"
set "BACKUP_DIR=%USERPROFILE%\KiwiTweaks_Backups\Backup_%dt:~0,8%_%dt:~8,6%"
mkdir "%BACKUP_DIR%" >nul 2>&1

echo Exporting TCP and Network Registry keys...
reg export "HKLM\SYSTEM\CurrentControlSet\Services\Tcpip\Parameters" "%BACKUP_DIR%\Tcpip_Parameters.reg" /y >nul 2>&1
echo Exporting Multimedia Scheduler keys...
reg export "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Multimedia\SystemProfile" "%BACKUP_DIR%\Multimedia_SystemProfile.reg" /y >nul 2>&1
echo Exporting GameDVR and FSE keys...
reg export "HKCU\System\GameConfigStore" "%BACKUP_DIR%\GameConfigStore.reg" /y >nul 2>&1
echo Exporting Mouse and Keyboard keys...
reg export "HKCU\Control Panel\Mouse" "%BACKUP_DIR%\Mouse_Settings.reg" /y >nul 2>&1
echo Exporting Memory Management keys...
reg export "HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Memory Management" "%BACKUP_DIR%\Memory_Management.reg" /y >nul 2>&1

echo.
echo %g%[SUCCESS]%w% Snapshot saved to: %BACKUP_DIR%
echo.
echo.                                              %b%??"????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
echo %b%^|%w%               Operation Completed, Press any key to continue...              %b%^|
echo.                                              %b%++
...
cls
echo.
echo Restoring latest Kiwi Settings Snapshot...
if not exist "%USERPROFILE%\KiwiTweaks_Backups" (
    echo %r%No backup snapshots found in %USERPROFILE%\KiwiTweaks_Backups%w%
    timeout /t 3 /nobreak > nul
    goto backup_menu
)
for /f "delims=" %%F in ('dir /b /ad /o-n "%USERPROFILE%\KiwiTweaks_Backups\Backup_*" 2^>nul') do (
    set "LATEST_BACKUP=%USERPROFILE%\KiwiTweaks_Backups\%%F"
    goto do_import
)
echo %r%No valid backup folders found.%w%
timeout /t 2 /nobreak > nul
goto backup_menu

:do_import
echo Importing registry files from: %LATEST_BACKUP%
if exist "%LATEST_BACKUP%\Tcpip_Parameters.reg" reg import "%LATEST_BACKUP%\Tcpip_Parameters.reg" >nul 2>&1
if exist "%LATEST_BACKUP%\Multimedia_SystemProfile.reg" reg import "%LATEST_BACKUP%\Multimedia_SystemProfile.reg" >nul 2>&1
if exist "%LATEST_BACKUP%\GameConfigStore.reg" reg import "%LATEST_BACKUP%\GameConfigStore.reg" >nul 2>&1
if exist "%LATEST_BACKUP%\Mouse_Settings.reg" reg import "%LATEST_BACKUP%\Mouse_Settings.reg" >nul 2>&1
if exist "%LATEST_BACKUP%\Memory_Management.reg" reg import "%LATEST_BACKUP%\Memory_Management.reg" >nul 2>&1
echo.
echo %g%[SUCCESS]%w% Latest snapshot successfully restored!
echo.
echo.                                              %b%??"????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
echo %b%^|%w%               Operation Completed, Press any key to continue...              %b%^|
echo.                                              %b%++
...
cls
echo.
echo Reverting All Tweaks to Windows Stock Defaults...
echo.
echo [1/5] Restoring Default Power Schemes...
powercfg -restoredefaultschemes >nul 2>&1
powercfg /setactive SCHEME_BALANCED >nul 2>&1

echo [2/5] Resetting TCP and Network Stack...
...
echo [3/5] Restoring Multimedia System Responsiveness...
reg add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Multimedia\SystemProfile" /v "SystemResponsiveness" /t REG_DWORD /d 20 /f >nul 2>&1
reg add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Multimedia\SystemProfile" /v "NetworkThrottlingIndex" /t REG_DWORD /d 10 /f >nul 2>&1

echo [4/5] Restoring Mouse Movement and Acceleration Defaults...
reg add "HKCU\Control Panel\Mouse" /v "MouseSensitivity" /t REG_SZ /d "10" /f >nul 2>&1
reg add "HKCU\Control Panel\Mouse" /v "MouseSpeed" /t REG_SZ /d "1" /f >nul 2>&1

echo [5/5] Re-enabling standard Windows Update services...
sc config wuauserv start= demand >nul 2>&1
sc config bits start= demand >nul 2>&1
sc config dosvc start= demand >nul 2>&1

echo.
echo %g%[SUCCESS]%w% System restored to Windows stock default settings!
echo.
echo.                                              %b%??"????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
echo %b%^|%w%               Operation Completed, Press any key to continue...              %b%^|
echo.                                              %b%++
...
echo %b%__________________________________________________________________________________
echo.
echo %w% Tweaking CPU Power Settings will increase fps, but also slightly increase temps
echo.
echo.
...
echo.
echo.
echo %w%Press "X" to go back to menu
echo.
echo %b%__________________________________________________________________________________
...
echo Invalid Input & goto Misspell_cpu
...
Reg.exe add "HKLM\SYSTEM\ControlSet001\Control\Processor" /v "CpuIdleScrubValueRead" /t REG_DWORD /d "0" /f
...
echo %b%__________________________________________________________________________________
echo.
echo %w%Optimize Windows Settings and More
echo.
echo.
...
echo Invalid Input & goto Misspell_wsettings
...
echo.
echo %b%__________________________________________________________________________________
echo %w%                Windows Game Mode
echo %b%__________________________________________________________________________________
echo.
echo %w%Turn off game mode if you are on win 10, turn it on on win 11
echo.
echo.
echo %p%[1]%w% ON       %p%[2]%w% Off
echo.
echo.
echo %w%Press "X" to go back to the menu
echo.
echo %b%__________________________________________________________________________________
...
echo.
echo %b%__________________________________________________________________________________
echo %w%                QOL (Quality of Life) Features
echo %b%__________________________________________________________________________________
echo %w%These may not improve performance, but they are nice qol features
echo.
...
echo Invalid Input & goto Misspell_More
...
echo.
chcp 437 > nul
powershell -Command "& {Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show('Disable everything in Playback and recording EXCEPT YOUR HEADPHONES AND MIC', 'Kiwis Tweaking Utility', 'Ok' , [System.Windows.Forms.MessageBoxIcon]::Information);}"
chcp 65001 > nul
...
echo %w%                Clean
echo %b%__________________________________________________________________________________
echo %w%These Will Clean All temporary files, log files etc...
echo.
echo.
...
if /i "%input%" == "5" goto Cleaner
if /i "%input%" == "6" goto cleanup
if /i "%input%" == "X" cls & goto menu

echo Invalid Input & goto Misspell_clean
...
echo %w%                Debloat
echo %b%__________________________________________________________________________________
echo %w%Debloat your pc, by removing useless apps, services and more!
echo.
echo.
...
echo Invalid Input & goto Misspell_debloat
...
:autoruns
chcp 437 > nul
powershell -Command "& {Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show('Unnecessary autoruns will be disabled now.\nOnly antivirus, cmd.exe, and N/A services will be kept.', 'Kiwis Tweaking Utility', 'OK', [System.Windows.Forms.MessageBoxIcon]::Information)}"
chcp 65001 > nul

echo Disabling unnecessary autorun entries...

for %%K in ("HKCU\Software\Microsoft\Windows\CurrentVersion\Run" "HKLM\Software\Microsoft\Windows\CurrentVersion\Run") do (
    for /f "tokens=1,2*" %%A in ('reg query %%K 2^>nul') do (
        echo Checking: %%C
        echo %%C ^| findstr /i "antivirus cmd.exe n/a" > nul
        if errorlevel 1 (
            echo Removing: %%C
            reg delete %%K /v %%~nxC /f > nul
        )
    )
)

set "StartupFolder=%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup"
set "StartupAll=%ProgramData%\Microsoft\Windows\Start Menu\Programs\Startup"

for %%F in ("%StartupFolder%\*" "%StartupAll%\*") do (
    echo %%~nxF ^| findstr /i "antivirus cmd.exe n/a" > nul
    if errorlevel 1 (
        echo Deleting startup shortcut: %%F
        del /f /q "%%~fF"
    )
)
...
PowerShell -Command "Get-AppxPackage Microsoft.MicrosoftEdge_41.16299.1004.0_neutral__8wekyb3d8bbwe|Remove-AppxPackage"
chcp 65001 > nul
echo.
echo.
echo.
...
echo.
echo %b%__________________________________________________________________________________
echo %w%                Visual Performance Tweaks
echo %b%__________________________________________________________________________________
...
echo Invalid Input & goto Misspell_visual
...
cls
chcp 437 > nul
powershell -Command "& {Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show('select adjust for best performanceAnd select Show Thumbnails Instead of icans, smooth edges on screen fonts and press apply', 'Kiwis Tweaking Utility', 'Ok', [System.Windows.Forms.MessageBoxIcon]::Information);}"
chcp 65001 > nul
...
echo %w%                GPU
echo %b%__________________________________________________________________________________
echo %w%Optimize GPU settings, only do general and the option where your gpu manufacturer is stated
echo.
echo.
...
echo Invalid Input & goto Misspell_gpu
...
echo Disable GpuEnergyDrvv
Reg.exe add "HKLM\SYSTEM\CurrentControlSet\Services\GpuEnergyDrvv" /v "Start" /t REG_DWORD /d "4" /f
...
cls
chcp 437 > nul
powershell -Command "& {Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show('ONLY DO THESE IF YOU HAVE AN NVIDIA GPU', 'Kiwis Tweaking Utility', 'Ok', [System.Windows.Forms.MessageBoxIcon]::Information);}"
chcp 65001 > nul
cls
...
echo %w%                Nvidia
echo %b%__________________________________________________________________________________
echo %w%Tweaking GPU settings will give a major FPS increase and a big decrease in GPU latency
echo.
echo %p%[1]%w%  Remove Nvidia Telemetry            %p%[2]%w%  Disable Hidden Power Saving Features
echo.
echo %p%[3]%w%  Enable GPU P-States                %p%[4]%w%  Enable MSI Mode and More
echo.
echo %p%[5]%w%  Disable TDR                        %p%[6]%w%  Disable HDCP
echo.
echo %p%[7]%w%  Disable All Preemptions            %p%[8]%w%  Enable DPC'S for each Core
echo.
echo %p%[9]%w%  Disable Write Combining            %p%[10]%w% Set Driver Acceleration
echo.
echo %p%[11]%w% Force Contiguous Memory Allocation %p%[12]%w% Advanced Driver Debloat
echo.
echo %p%[13]%w% Advanced Tweaks                    %p%[14]%w% Optimize PowerMizer
echo.
echo %p%[15]%w% Latency Tolerance
echo.
echo %w%Type "X" to go back to menu
echo.
echo %b%__________________________________________________________________________________
...
if /i "%input%" == "3" goto nv4
if /i "%input%" == "4" goto nv5
if /i "%input%" == "5" goto nv6
if /i "%input%" == "6" goto nv7
if /i "%input%" == "7" goto nv8
if /i "%input%" == "8" goto nv9
if /i "%input%" == "9" goto nv10
if /i "%input%" == "10" goto nv11
if /i "%input%" == "11" goto nv12
if /i "%input%" == "12" goto nv13
if /i "%input%" == "13" goto nv14
if /i "%input%" == "14" goto nv15
if /i "%input%" == "15" goto nv16
if /i "%input%" == "X" cls & goto menu
echo Invalid Input & goto Misspell_nvgpu
...
for /f %%i in ('wmic path Win32_VideoController get PNPDeviceID') do set "str=%%i" (
    ...
    Reg.exe add "HKLM\System\CurrentControlSet\Enum\%%i\Device Parameters\Interrupt Management\Affinity Policy" /v "DevicePriority" /t REG_DWORD /d "0" /f
    timeout /t 1 /nobreak > NUL
...
cls
chcp 437 > nul
powershell -Command "& {Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show('ONLY DO THESE IF YOU HAVE AN AMD GPU', 'Kiwis Tweaking Utility', 'Ok', [System.Windows.Forms.MessageBoxIcon]::Information);}"
chcp 65001 > nul
...
for /f %%w in ('reg query "HKLM\SYSTEM\CurrentControlSet\Control\Class\{4d36e968-e325-11ce-bfc1-08002be10318}" /s /v "DriverDesc"^| findstr "HKEY AMD ATI"') do if /i "%%w" neq "DriverDesc" (set "REGPATH_AMD=%%w")
        ...
        Reg.exe add "!REGPATH_AMD!" /v "DisableBlockWrite" /t REG_DWORD /d "0" /f
...
:intelgpu
chcp 437 > nul
powershell -Command "& {Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show('ONLY DO THESE IF YOU HAVE AN INTEL GPU', 'Kiwis Tweaking Utility', 'Ok', [System.Windows.Forms.MessageBoxIcon]::Information);}"
chcp 65001 > nul
cls
...
echo %w%                Usb
echo %b%__________________________________________________________________________________
echo %w%These decrease USB latency and improve performance
echo.
echo.
...
echo Invalid Input & goto Misspell_usb
...
echo %w%                Advanced
echo %b%__________________________________________________________________________________
echo %w% This is a bunch of advanced optimzations that will help with Performance and latency
echo.
echo.
...
echo.
echo.
echo %w% Type "X" to go back to the menu
echo.
echo %b%__________________________________________________________________________________
...
echo Invalid Input & goto Misspell_god
...
echo.
echo %b%__________________________________________________________________________________
echo %w%                Monitor Refresh Rate
echo %b%__________________________________________________________________________________
echo.
echo %w% Select Your Monitors Refresh rate, this will lock it to it!
echo.
echo.
...
if /i "%input%" == "X" cls & goto god

echo Invalid Input & goto Misspell_god23
...
:: Safe OneDrive unlinking and policy disable (preserves personal user files)
...
:: IFEO wipe omitted to protect critical system execution options
chcp 65001 > nul
sc stop SysMain
...
:wub
chcp 437 > nul
powershell -Command "& {Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show('Select disable updates in WUB, press OK to open it', 'Kiwis Tweaking Utility', 'OK', [System.Windows.Forms.MessageBoxIcon]::Information)}"
chcp 65001 > nul
...
reg add "HKLM\SOFTWARE\Microsoft\WindowsUpdate\UX\Settings" /v RestartPending /t REG_DWORD /d 0 /f
...
echo %b%__________________________________________________________________________________
echo %w%                Disk Management
echo %b%__________________________________________________________________________________
echo %w% These Commands Fix corrupted files
echo.
echo.
...
echo Invalid Input & goto Misspell_fix
...
echo %w%                RAM
echo %b%__________________________________________________________________________________
echo %w% Ram Tweaks help to lower ram usage and responsivness which increases fps
echo.
echo.
...
echo Invalid Input & goto Misspell_ram
...
:: 32-bit PAE key omitted for 64-bit stability
Reg.exe add "HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Memory Management" /v "ClearPageFileAtShutdown" /t REG_DWORD /d "0" /f
Reg.exe add "HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Memory Management" /v "DisablePagingExecutive" /t REG_DWORD /d "1" /f
Reg.exe add "HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Memory Management" /v "LargeSystemCache" /t REG_DWORD /d "0" /f
Reg.exe add "HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Memory Management" /v "NonPagedPoolQuota" /t REG_DWORD /d "0" /f
...
if /i "%input%" == "X" cls & goto menu
echo Invalid Input & goto Misspell_storage
...
echo %w%                fsutil
echo %b%__________________________________________________________________________________
echo %w% Select if your boot drive is a ssd or a hdd
echo.
echo.
...
if /i "%input%" == "X" cls & goto menu
echo Invalid Input & goto Misspell_fsutil
...
echo %w%                Priority
echo %b%__________________________________________________________________________________
echo %w% These will make sure that your pc uses its rescourses on things that matter!
echo.
echo.
echo %p%[1]%w% Optimize MMCSS                   %p%[2]%w% Service Priority
echo.
echo %p%[3]%w% Game Priority                    %p%[4]%w% Win32 Priority Separation
echo.
echo %p%[5]%w% Thread Priority                  %p%[6]%w% System Responsiveness
echo.
echo %p%[7]%w% IRQ8 Priority                    %p%[8]%w% IRQ16 Priority
echo.
echo %p%[9]%w% CSRSS Priority                    %p%[10]%w% Window Manager Priority
echo.
...
echo Invalid Input & goto Misspell_priority
...
:pr11
set "mitigation_mask=222222222222222222222222222222222222222222222222"
for %%a in (
    fontdrvhost.exe
    dwm.exe
    lsass.exe
...
echo.
echo %b%__________________________________________________________________________________
echo %w%                Priority (Game specific)
echo %b%__________________________________________________________________________________
echo.
echo %w% Game specific Priority Tweaks Apply Cpu, Gpu and Network Priority to specific games,
echo %w% (adding more games in the future)
echo.
echo.
...
echo Invalid Input & goto Misspell_gamepriority

:Misspell_gamepriority
cls
echo Misspell Detected
timeout /t 2 /nobreak > nul
cls
...
Reg.exe add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\GTA5.exe\PerfOptions" /v "GpuPriorityClass" /t REG_DWORD /d "3" /f
...
Reg.exe add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\fivem.exe\PerfOptions" /v "GpuPriorityClass" /t REG_DWORD /d "3" /f
...
Reg.exe add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\cs2.exe\PerfOptions" /v "GpuPriorityClass" /t REG_DWORD /d "3" /f
...
Reg.exe add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\r5apex.exe\PerfOptions" /v "GpuPriorityClass" /t REG_DWORD /d "3" /f
...
Reg.exe add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\FortniteClient-Win64-Shipping.exe\PerfOptions" /v "GpuPriorityClass" /t REG_DWORD /d "3" /f
...
Reg.exe add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\ValorantClient-Win64-Shipping.exe\PerfOptions" /v "GpuPriorityClass" /t REG_DWORD /d "3" /f
...
:cod
for %%i in (cod.exe) do (
    Reg.exe add "HKLM\Software\Policies\Microsoft\Windows\QoS\%%~ni" /v "Application Name" /t REG_SZ /d "%%i" /f
    Reg.exe add "HKLM\Software\Policies\Microsoft\Windows\QoS\%%~ni" /v "Version" /t REG_SZ /d "1.0" /f
    Reg.exe add "HKLM\Software\Policies\Microsoft\Windows\QoS\%%~ni" /v "Protocol" /t REG_SZ /d "*" /f
    Reg.exe add "HKLM\Software\Policies\Microsoft\Windows\QoS\%%~ni" /v "DSCP Value" /t REG_SZ /d "46" /f
    Reg.exe add "HKLM\Software\Policies\Microsoft\Windows\QoS\%%~ni" /v "Throttle Rate" /t REG_SZ /d "-1" /f
)
...
:r6
for %%i in (RainbowSix.exe RainbowSix_Vulkan.exe) do (
    Reg.exe add "HKLM\Software\Policies\Microsoft\Windows\QoS\%%~ni" /v "Application Name" /t REG_SZ /d "%%i" /f
    Reg.exe add "HKLM\Software\Policies\Microsoft\Windows\QoS\%%~ni" /v "Version" /t REG_SZ /d "1.0" /f
    Reg.exe add "HKLM\Software\Policies\Microsoft\Windows\QoS\%%~ni" /v "Protocol" /t REG_SZ /d "*" /f
    Reg.exe add "HKLM\Software\Policies\Microsoft\Windows\QoS\%%~ni" /v "DSCP Value" /t REG_SZ /d "46" /f
    Reg.exe add "HKLM\Software\Policies\Microsoft\Windows\QoS\%%~ni" /v "Throttle Rate" /t REG_SZ /d "-1" /f
    Reg.exe add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\%%i\PerfOptions" /v "CpuPriorityClass" /t REG_DWORD /d "3" /f
    Reg.exe add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\%%i\PerfOptions" /v "IoPriority" /t REG_DWORD /d "3" /f
)
echo.
echo.                                              %b%??"????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
echo %b%^|%w%               Operation Completed, Press any key to continue...              %b%^|
...
Reg.exe add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\Overwatch.exe\PerfOptions" /v "CpuPriorityClass" /t REG_DWORD /d "3" /f
...
Reg.exe add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\RustClient.exe\PerfOptions" /v "CpuPriorityClass" /t REG_DWORD /d "3" /f
Reg.exe add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\RustClient.exe\PerfOptions" /v "IoPriority" /t REG_DWORD /d "3" /f
echo.
echo.                                              %b%??"????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
echo %b%^|%w%               Operation Completed, Press any key to continue...              %b%^|
echo.                                              %b%++
...
:roblox
for %%i in (RobloxPlayerBeta.exe) do (
    Reg.exe add "HKLM\Software\Policies\Microsoft\Windows\QoS\%%~ni" /v "Application Name" /t REG_SZ /d "%%i" /f
    Reg.exe add "HKLM\Software\Policies\Microsoft\Windows\QoS\%%~ni" /v "Version" /t REG_SZ /d "1.0" /f
    Reg.exe add "HKLM\Software\Policies\Microsoft\Windows\QoS\%%~ni" /v "Protocol" /t REG_SZ /d "*" /f
    Reg.exe add "HKLM\Software\Policies\Microsoft\Windows\QoS\%%~ni" /v "DSCP Value" /t REG_SZ /d "46" /f
...
Reg.exe add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\RobloxPlayerBeta.exe\PerfOptions" /v "IoPriority" /t REG_DWORD /d "3" /f
echo.
echo.                                              %b%??"????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
echo %b%^|%w%               Operation Completed, Press any key to continue...              %b%^|
echo.                                              %b%++
...
:lol
for %%i in ("League of Legends.exe") do (
    Reg.exe add "HKLM\Software\Policies\Microsoft\Windows\QoS\LeagueOfLegends" /v "Application Name" /t REG_SZ /d "%%~i" /f
    Reg.exe add "HKLM\Software\Policies\Microsoft\Windows\QoS\LeagueOfLegends" /v "Version" /t REG_SZ /d "1.0" /f
    Reg.exe add "HKLM\Software\Policies\Microsoft\Windows\QoS\LeagueOfLegends" /v "Protocol" /t REG_SZ /d "*" /f
    Reg.exe add "HKLM\Software\Policies\Microsoft\Windows\QoS\LeagueOfLegends" /v "DSCP Value" /t REG_SZ /d "46" /f
    Reg.exe add "HKLM\Software\Policies\Microsoft\Windows\QoS\LeagueOfLegends" /v "Throttle Rate" /t REG_SZ /d "-1" /f
)
...
echo %w%                Power
echo %b%__________________________________________________________________________________
echo %w% Tweaking Power Settings will increase fps, but also slightly increase temps
echo.
echo.
echo %p%[1]%w% Set Power Plan                         %p%[2]%w% Additional Power Tweaks
echo.
echo %p%[3]%w% Enable Kboost                          %p%[4]%w% Disable Storage Power Management
echo.
echo %p%[5]%w% Enable Hardware P-states               %p%[6]%w% Disable Power Throttling
echo.
echo %p%[7]%w% Disable Idle Power Management           %p%[8]%w% Disable Link Power Management
echo.
...
echo.
echo %p%[15]%w% Disable Sleep Study and Hibernation  %p%[16]%w% Disable Power Estimation and Telemetry
echo.
...
echo Invalid Input & goto Misspell_power
...
echo Applying Windows High Performance power plan...
...
Reg.exe add "HKLM\SYSTEM\CurrentControlSet\Control\Power\PowerSettings\19cbb8fa-5279-450e-9fac-8a3d5fedd0c1\12bbebe6-58d6-4636-95bb-3217ef867c1a" /v "Attributes" /t REG_DWORD /d "2" /f
Reg.exe add "HKLM\SYSTEM\CurrentControlSet\Control\Power\PowerSettings\245d8541-3943-4422-b025-13a784f679b7" /v "Attributes" /t REG_DWORD /d "2" /f
...
Reg.exe add "HKLM\SYSTEM\CurrentControlSet\Control\Power\PowerSettings\2a737441-1930-4402-8d77-b2bebba308a3\48e6b7a6-50f5-4782-a5d4-53bb8f07e226" /v "Attributes" /t REG_DWORD /d "2" /f
...
Reg.exe add "HKLM\SYSTEM\CurrentControlSet\Control\Power\PowerSettings\44f3beca-a7c0-460e-9df2-bb8b99e0cba6\3619c3f2-afb2-4afc-b0e9-e7fef372de36" /v "Attributes" /t REG_DWORD /d "2" /f
...
Reg.exe add "HKLM\SYSTEM\CurrentControlSet\Control\Power\PowerSettings\501a4d13-42af-4429-9fd1-a8218c268e20\ee12f906-d277-404b-b6da-e5fa1a576df5" /v "Attributes" /t REG_DWORD /d "2" /f
Reg.exe add "HKLM\SYSTEM\CurrentControlSet\Control\Power\PowerSettings\5FB4938D-1EE8-4b0f-9A3C-5036B0AB995C\DD848B2A-8A5D-4451-9AE2-39CD41658F6C" /v "Attributes" /t REG_DWORD /d "2" /f
...
Reg.exe add "HKLM\SYSTEM\CurrentControlSet\Control\Power\PowerSettings\9596FB26-9850-41fd-AC3E-F7C3C00AFD4B\10778347-1370-4ee0-8bbd-33bdacaade49" /v "Attributes" /t REG_DWORD /d "2" /f
Reg.exe add "HKLM\SYSTEM\CurrentControlSet\Control\Power\PowerSettings\9596FB26-9850-41fd-AC3E-F7C3C00AFD4B\34C7B99F-9A6D-4b3c-8DC7-B6693B78CEF4" /v "Attributes" /t REG_DWORD /d "2" /f
...
Reg.exe add "HKLM\SYSTEM\CurrentControlSet\Control\Power\PowerSettings\54533251-82be-4824-96c1-47b60b740d00\94D3A615-A899-4AC5-AE2B-E4D8F634367F" /v "Attributes" /t REG_DWORD /d "2" /f
...
echo %w%                FSE
echo %b%__________________________________________________________________________________
echo %w% Full Screen Optimization and GameDVR
echo.
echo.
...
echo Invalid Input & goto Misspell_fse
...
echo %w%                KBM
echo %b%__________________________________________________________________________________
echo %w% These Will drastically decrease input delay,
echo %w% Also make your keyboard and mouse feel more responsive.
echo.
echo.
...
echo Invalid Input & goto Misspell_mouse
...
echo %w%                KBM
echo %b%__________________________________________________________________________________
echo %w% Low mouse data queue size lowers input delay, but it will cause mouse and kb lag on low end pcs
echo %w% The Lower The Number, The Lower the delay, but more lag on bad pcs
echo.
echo.
...
echo Invalid Input & goto Misspell_data
...
:: Driver integrity services preserved for anti-cheat compatibility
bcdedit /set integrityservices enable
bcdedit /set bootux disabled
bcdedit /set bootlog no
bcdedit /set tpmbootentropy ForceDisable
:: ELAM drivers preserved for secure boot compatibility
...
cls
chcp 437 > nul
powershell -Command "& {Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show('ONLY DO THESE ON ETHERNET, WIFI TWEAKS COMING IN 1.0', 'Kiwis Tweaking Utility', 'Ok', [System.Windows.Forms.MessageBoxIcon]::Information);}"
chcp 65001 > nul
echo
echo.
echo.
...
echo %w%                Network
echo %b%__________________________________________________________________________________
echo %w% ONLY DO THESE ON ETHERNET
echo %w% These tweaks decrease ping, decrease bufferbloat and increase internet speeds
echo.
echo.
echo %p%   %w%                                       %p%[0]%w% Fully Reset Network Settings
echo.
echo %p%[1]%w% Disable Hidden Power Saving Features   %p%[2]%w% Turn off limiting             %p%[3]%w% Optimize Network Adapter
echo.
echo %p%[4]%w% Registry Tweaks                       %p%[5]%w% Disable Nagle's Algorithm     %p%[6]%w% Enable Direct Cache Access
echo.
echo %p%[7]%w% DNS Priority                          %p%[8]%w% Enable Onboard Processor      %p%[9]%w% Enable MSI Mode On network adapter
echo.
echo %p%[10]%w% Disable Netbios over TCPIP           %p%[11]%w% Network Throttling Index     %p%[12]%w% Disable Wi-Fi (Wireless Network)
echo.
...
echo Invalid Input & goto Misspell_net
...
echo %w%                Network
echo %b%__________________________________________________________________________________
echo %w% Disable Wifi Services, ethernet will still work ofc
echo.
echo.
...
echo Invalid Input & goto Misspell_wifi
...
nbtstat -R
nbtstat -RR
:: netcfg -d omitted to preserve VPN and virtual adapter bindings
netsh winsock reset
...
:: Outdated 2012 YouTube IP block removed
timeout /t 1 /nobreak > nul
...
:: Dial-up TCP window cap removed for full broadband speeds
:: Dial-up TCP window cap removed for full broadband speeds
